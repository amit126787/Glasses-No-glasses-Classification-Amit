...
def zip_answer(answer):
    """
    this function is checking if the input the the user entered is valid if not he need to enter the input again
    :param answer: y/n input from the user
    :return: sends to open the zip file if requsted if not exit
    """
    b = True  # to end the loop
    while b:
        if answer != "y" and answer != "n":
            ...
            b = False
    if answer == "y":
        answer = input("input zip path")
        while not os.path.exists(answer):
            answer = input("input a valid zip path")
        open_zip(answer)
